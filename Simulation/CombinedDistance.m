...
for kk = 1:N
    ...
    measurements = CalculateCombinedDistancesM(XCVRTarget, XCVRBase);
    error = randn(1000,40)'*0.01;
    for i = 1:4
        error((i-1)*10+5,:)  = error((i-1)*10+1,:) - error((i-1)*10+2,:);
        error((i-1)*10+6,:)  = error((i-1)*10+1,:) - error((i-1)*10+3,:);
        error((i-1)*10+7,:)  = error((i-1)*10+1,:) - error((i-1)*10+4,:);
        error((i-1)*10+8,:)  = error((i-1)*10+2,:) - error((i-1)*10+3,:);
        error((i-1)*10+9,:)  = error((i-1)*10+2,:) - error((i-1)*10+4,:);
        error((i-1)*10+10,:) = error((i-1)*10+3,:) - error((i-1)*10+4,:);
    end
    ...
    for k=1:20
        ...
        estDistances  = CalculateCombinedDistances(estXCVRTarget, XCVRBase);
        distanceError = measurements - estDistances;
        distanceError(42) = 1 - sum(m(4:7).^2);   % norm(q) = 1
        distanceError(42) = 1 - sum(m(4:7).^2)^3; % determinant(R) = 1

        % Calculate Jacobian
        for i=1:7
            diffOffset = circshift([epsilon;0;0;0;0;0;0],i-1);
            pointsTemp =  EstimateTargetPoints(m + diffOffset, 4, XCVRBase);
            G(1:40,i)  = (CalculateCombinedDistances(pointsTemp, XCVRBase) - ...
                          estDistances) / epsilon;
        end
        G(41,4:7)    = 2 * m(4:7);                    % norm(q) = 1
        G(42,4:7)    = 6 * sum(m(4:7).^2)^2 * m(4:7); % determinant(R) = 1
...
suptitle('Combined Distance Measurements (1000 Samples)')
